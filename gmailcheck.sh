...
pw1=$(gpg2 -dq $HOME/.mutt_password.gpg | awk 'NR==1 {print $4}')

#user2=$(cat $HOME/.mutt/account.2.muttrc | grep -m 1 "imap_user" | awk '{gsub("\"", "", $4); print $4}')
#pw2=$(gpg2 -dq $HOME/.mutt_password.gpg | awk 'NR==2 {print $4}')
...
full_newmail_count=$(( $mail_1_newmail_count))
...
if [[ $full_newmail_count -ge "1" ]]; then
    ...
    echo "mail_widget:set_markup(\"<span color='#FFFFFF'>Mail: </span><span background='#C90303' color='#ffffff'> ${full_newmail_count} mail </span>\")" | awesome-client
else
